/**
 * PlaybackHost - компонент для режима playback (основной режим).
 * 
...
import { computeLayout, type Layout } from '../renderer/layout/layout';
import * as PIXI from 'pixi.js';
import { GuardfolioVideoProgram } from '../programs';
import { DebugHUD } from './DebugHUD';

export interface PlaybackHostConfig {
  quality?: 'low' | 'medium' | 'high' | 'ultra';
  width?: number;
  height?: number;
}

export function PlaybackHost({ quality = 'high', width = 1920, height = 1080 }: PlaybackHostConfig = {}) {
  ...
  const [program, setProgram] = useState<GuardfolioVideoProgram | null>(null);

  // Создаём программу
  useEffect(() => {
    let mounted = true;

    GuardfolioVideoProgram.create()
      .then((createdProgram) => {
        if (!mounted) return;
      ...
      .catch((error) => {
        console.error('[PlaybackHost] Failed to create program:', error);
      });
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        // 3. Создаём TimeSource (real-time для playback)
        ...
        setIsReady(true);
        console.log('[PlaybackHost] Initialized:', { quality, duration: program.getDuration() });
      } catch (error) {
        console.error('[PlaybackHost] Initialization error:', error);
      }
    };
  ...
  return (
    <>
      ...
      />
      <DebugHUD scene={null} isRecording={false} />
    </>
  );
}
