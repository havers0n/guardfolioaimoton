/**
 * SceneInspector - инструмент для отладки и инспекции сцены.
 * Позволяет:
...
import { GuardfolioScene } from '../scenes/guardfolio/GuardfolioScene';
...
export function SceneInspector({ scene }: { scene: Scene }) {
  const [elements, setElements] = useState<ElementInfo[]>([]);
  const [phases, setPhases] = useState<PhaseInfo[]>([]);
  const [currentTime, setCurrentTime] = useState(0);
  const [activeShot, setActiveShot] = useState<SceneShot | null>(null);
  const [shotsEnabled, setShotsEnabled] = useState(true); // Флаг для включения/выключения управления через шоты
  ...
  useEffect(() => {
    if (!(scene instanceof GuardfolioScene)) return;

    const guardfolioScene = scene as GuardfolioScene;
    const renderer = guardfolioScene.getRenderer();
    const engine = guardfolioScene.getEngine();

    if (!renderer || !engine) return;

    // Получаем элементы
    const elementList = renderer.getElements();
    const elementInfos: ElementInfo[] = elementList.map((item, index) => ({
      index,
      type: item.type,
      visible: renderer.getElementVisible(index),
      bounds: renderer.getElementBounds(index),
    }));
    setElements(elementInfos);

    // Получаем фазы
    const phaseList = guardfolioScene.getPhases();
    setPhases(phaseList);
    ...
    const mapShotElementTypeToRendererType = (shotType: keyof SceneShot['visible']): string | null => {
      switch (shotType) {
        case 'header':
          return 'overlaytext'; // overlay-text становится overlaytext в getElements
        case 'chart':
          return 'chart';
        case 'logo':
          return 'logo';
        case 'background':
          return 'background';
        case 'tasks':
          // tasks может быть частью chart или отдельным элементом
          // Пока считаем, что это часть chart, но можно расширить
          return null; // Не управляем отдельно, если нет отдельного элемента
        default:
          return null;
      }
    };
    ...
    const applyShotVisibility = (shot: SceneShot | null, elementList: Array<{ element: any; container: PIXI.Container; type: string }>, enabled: boolean) => {
      ...
      // Если нет активного шота, скрываем все элементы кроме background
      if (!shot) {
        elementList.forEach((item, index) => {
          if (item.type !== 'background') {
            renderer.setElementVisible(index, false);
          }
        });
        return;
      }

      // Применяем видимость для каждого элемента на основе активного шота
      Object.entries(shot.visible).forEach(([shotElementType, shouldBeVisible]) => {
        ...
        // Находим элемент по типу
        const elementIndex = elementList.findIndex(item => item.type === rendererType);
        if (elementIndex >= 0) {
          const currentVisible = renderer.getElementVisible(elementIndex);
          if (currentVisible !== shouldBeVisible) {
            renderer.setElementVisible(elementIndex, shouldBeVisible ?? false);
          }
        }
      });
    };

    // Применяем начальную видимость на основе текущего времени
    ...
    // Создаем debug overlay для отрисовки bounds и safe-area
    const app = renderer.getApplication();
    if (app && app.stage) {
      const overlay = new PIXI.Graphics();
      overlay.zIndex = 10000; // Поверх всего
      app.stage.addChild(overlay);
      debugOverlayRef.current = overlay;
      setDebugOverlay(overlay);

      // Сортируем children по zIndex
      app.stage.children.sort((a, b) => (a.zIndex || 0) - (b.zIndex || 0));
    }
    ...
    const updateInterval = setInterval(() => {
      ...
      // Определяем активный шот
      const currentShot = getActiveShot(time);
      setActiveShot(currentShot);

      // Обновляем bounds элементов
      const currentElementList = renderer.getElements();
      
      // Применяем видимость на основе активного шота
    ...
    const drawDebugOverlay = () => {
      ...
      // Рисуем safe-area через layout
      if (showSafeArea && layout) {
        // Safe area из layout (только зеленые линии, без красной заливки для устранения дублирования)
        overlay.rect(
          layout.safe.left,
          layout.safe.top,
      ...
      // Рисуем anchors (якоря: center, columns)
      if (showAnchors && layout) {
        const anchorSize = 8;
        const anchorColor = 0x10b981; // green-500
        ...
        overlay.fill({ color: 0x3b82f6, alpha: 0.8 }); // blue
        
        overlay.circle(layout.centerColX, layout.centerY, anchorSize * 0.7);
        overlay.fill({ color: 0x8b5cf6, alpha: 0.8 }); // purple
        
        overlay.circle(layout.rightColX, layout.centerY, anchorSize * 0.7);
        overlay.fill({ color: 0xef4444, alpha: 0.8 }); // red
      ...
      // Рисуем bounding boxes элементов
      if (showBounds) {
        elementList.forEach((item, index) => {
          const bounds = renderer.getElementBounds(index);
          if (!bounds || bounds.width === 0 || bounds.height === 0) return;

          const visible = renderer.getElementVisible(index);
    ...
    // Запускаем отрисовку debug overlay
    drawDebugOverlay();
    ...
    return () => {
      ...
      if (debugOverlayRef.current && app && app.stage) {
        app.stage.removeChild(debugOverlayRef.current);
        debugOverlayRef.current.destroy();
        debugOverlayRef.current = null;
      }
    };
  }, [scene, showBounds, showSafeArea, showGrid, showAnchors, shotsEnabled]);
  ...
  const toggleElementVisibility = (index: number) => {
    ...
    const currentVisible = renderer.getElementVisible(index);
    renderer.setElementVisible(index, !currentVisible);
